...
const ALLOWED_ORIGINS = new Set([
    ...
    'https://matchmentum-staging.netlify.app'
]);

function corsHeaders(originHeader) {
    // Fall back to the staging origin, not production, when the request
    // origin isn't recognized — so an unmatched/misconfigured origin fails
    // visibly in staging testing rather than silently passing in prod.
    const origin = ALLOWED_ORIGINS.has(originHeader) ? originHeader : 'https://matchmentum-staging.netlify.app';
    return {
